...
$a = Get-ChildItem *.csv | Where-Object name -ne 'Bulk User Sheet.csv' | Select-Object -exp Fullname
$a | ForEach-Object {
...
foreach ($account in $csv) {
    if ($account.'C-Number' -match 'C-.+') {
        ...
        groups $template | ForEach-Object { Add-ADPrincipalGroupMembership $userid -MemberOf $_ -Verbose 2>$null }
